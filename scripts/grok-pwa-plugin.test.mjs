...
test("site title Kalendár is a real name, not a sentinel", () => {
  const out = injectGrokPwaHead("<html><head></head></html>", {
    host: "wild-race.grok.me",
    site: { title: "Kalendár" },
  });
  assert.match(out, /property="og:title" content="Kalendár"/);
});
...
test("names the install page from host slug", () => {
  assert.equal(appNameFromHost("localhost:8080"), "Kalendár Zmien");
  assert.equal(appNameFromHost("172.17.154.217:8080"), "Kalendár Zmien");
  assert.equal(appNameFromHost("wild-race.grok.me"), "Wild Race");
});

test("rejects hosts that are not plain slugs", () => {
  assert.equal(appNameFromHost("<script>alert(1)</script>"), "Kalendár Zmien");
  assert.equal(appNameFromHost('"><img src=x onerror=1>.grok.me'), "Kalendár Zmien");
});
